...
public class SimpleDomainExtractor {
    ...
    /**
     * Extracts domain and path of a URI without adding the name of the
     * resource. If "http://example.org/resource/test" is used as input this
     * method will return "example.org".
     * 
     * @param uri
     *            the URI as String
     * @return the extracted domain and path
     */
     ...
     * Extracts domain and path of a URI without adding the name of the
     * resource. If "http://example.org/resource/test" is used as input this
     * method will return "example.org/resource/".
     * 
     * @param uri
     *            the URI as String
     * @return the extracted domain and path
     */
    public static String extractDomainAndPath(String uri) {
        if (uri == null) {
            return null;
        }
        // get the start position of the domain
        ...
        // find the end position of the String
        char chars[] = uri.toCharArray();
        for (int i = (chars.length - 1); i > startPos; --i) {
            switch (chars[i]) {
            // if this is a character that is not part of the resource, anymore
            case '/':
            case '#': {
                return uri.substring(startPos, i);
            }
